...
import { verifyAccountApi, type VerifyAccountPayload } from "@/features/auth/api/auth.api";
  ...
  return useMutation({
    ...
    onError: (error: Error) => {
      if (error instanceof ApiError && error.statusCode === 422) {
        ...
        toast({ type: "error", title: "Verification Failed", description: (error as Error).message });
      }
    },
  });
}
