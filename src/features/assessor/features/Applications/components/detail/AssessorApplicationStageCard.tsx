...
> = ({ stage, onViewApplicationForm, onOpenEvidenceVault }) => {
  ...
  const isCollapsible = stage.isCollapsible && !stage.menuActions;
  const isViewStage = stage.actionButton?.variant === "view";
  const handleCardClick = isViewStage ? (stage.actionButton?.onClick || onViewApplicationForm) : undefined;

  return (
    <div
      onClick={handleCardClick}
      className={`bg-white rounded-2xl p-5 sm:p-6 border border-gray-100 shadow-xs flex flex-col gap-3 transition-all relative ${
