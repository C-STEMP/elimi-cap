...
> = ({ onSelectApplication }) => {
  ...
  const applications: AssessorApplicationRecord[] = useMemo(() => {
    return apiApplications ? apiApplications.map(mapApplicationToRecord) : [];
  }, [apiApplications]);

  // Trade filter options are derived from the trades actually present in
  // this assessor's real assignments, not a fixed catalogue-wide list.
  const availableTrades = useMemo(() => {
    const trades = new Set<string>();
    applications.forEach((app) => {
      if (app.trade) trades.add(app.trade);
    });
