      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoadingInterviews
          ...
          ? filteredInterviews.map((item) => (
              <div key={item.id} className="bg-white rounded-2xl p-5 border border-black/20 shadow-2xs hover:shadow-xs transition-all flex flex-col gap-3">
                <div className="flex items-start justify-between gap-3">
                  <input type="checkbox" checked={selectedInterviewIds.includes(item.id)} onChange={() => onToggleSelect(item.id)} className="w-4 h-4 rounded border-gray-300 text-[#a31d38] cursor-pointer" />
                ...
                <div className="flex flex-col gap-1.5">
                  <span onClick={() => handleView(item)} className="font-bold text-sm text-black cursor-pointer hover:text-primary transition-colors">{item.title}</span>
                ...
                <button type="button" onClick={() => handleView(item)} className="text-xs text-black font-bold underline hover:text-[#a31d38] cursor-pointer mt-auto self-start">View</button>
              </div>
            ))
          ...
          {isLoadingInterviews
            ...
            ? filteredInterviews.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50/70 transition-colors">
                  <td className="p-4"><input type="checkbox" checked={selectedInterviewIds.includes(item.id)} onChange={() => onToggleSelect(item.id)} className="w-4 h-4 accent-primary rounded cursor-pointer" /></td>
                  <td onClick={() => handleView(item)} className="p-4 font-semibold text-black cursor-pointer hover:text-primary">{item.title}</td>
                  ...
                  <td className="p-4 text-right"><button type="button" onClick={() => handleView(item)} className="font-semibold text-black underline underline-offset-2 hover:text-primary cursor-pointer">View</button></td>
                </tr>
              ))
